...
# API Key for authentication (read from environment)
...
async def verify_api_key(x_api_key: Optional[str] = Header(None)):
    ...
    Applied only to the /score endpoint.
    """
    if API_KEY and API_KEY.strip():  # only enforce if a key is configured
...
class ConnectionManager:
    ...
    async def broadcast(self, message: Dict[str, Any]):
        """Broadcast message to all connected clients concurrently using asyncio.gather."""
        ...
        async with self._lock:
            ...
            async def _safe_send(connection: WebSocket) -> Optional[WebSocket]:
                """Send to a single client; return the connection on failure so we can clean up."""
                try:
                    await connection.send_json(message)
                    return None  # success
                except Exception as e:
                    logger.warning(f"Failed to send to client: {e}")
                    return connection  # mark for removal
            ...
            for failed_conn in results:
                if failed_conn is not None and failed_conn in self.active_connections:
                    self.active_connections.remove(failed_conn)
